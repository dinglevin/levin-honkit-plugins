...
function wrapIfHint(body, hintStyle) {
  if (hintStyle) {
    return `
        <div class="alert alert-${ALERT_STYLES[hintStyle]} hints-alert">
  ...
  blocks: {
    tooltips: {
      process: async function (block) {
        ...
        const delimiter =
          block.kwargs?.delimiter || pluginConfig?.delimiter || '@@';
        ...
        const rendredTooltipsText = await this.book.renderBlock(
          'markdown',
          tooltipstext
        );
